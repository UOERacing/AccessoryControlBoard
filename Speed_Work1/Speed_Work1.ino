...
const int hallPin = 2 ; //Pin which goes HIGH when the magnetised bolt passes in front of the sensor. Make sure it can use an interrupt.
const int bounce_duration = 100; //Estimated button bounce duration for software debouncing (I stole this off the internet and don't fully
                                 // understand how it works but it does).
...
//Interrupt :
    attachInterrupt(digitalPinToInterrupt(hallPin),recordTime,RISING);
}
...
//It also initiates a new cycle based on the previous final time.
  void calculateSpeed(){
    ...
    if(pointInCycle == 2){
      ...
      speed = (1.00/(millisecondsPerRev/1000.00))*2*pi; //This part gives the angular velocity in radians/second.
